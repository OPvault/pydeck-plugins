...
The Discord plugin connects to the **local Discord desktop client** over Discord's IPC endpoint (no cloud request on every button press) — a Unix domain socket on Linux and macOS, a named pipe on Windows. It uses OAuth2 to obtain an access token with the `rpc`, `rpc.voice.read`, and `rpc.voice.write` scopes, then keeps a persistent authenticated connection open so toggle actions are instant.
...
> **The Discord plugin uses Discord's IPC endpoint, which is only exposed by the official desktop client.**
>
> The following will **not** work:
> - **Browser Discord** (`discord.com/app`) — no IPC endpoint is available in a browser tab
> - **Vencord, BetterDiscord, or any other modified client** — these clients do not reliably expose the IPC endpoint that the RPC protocol depends on
...
2. Under **Redirects** click **Add Redirect** and enter exactly:
   ```text
   http://127.0.0.1:8686/oauth/no.pydeck.discord/callback
   ```

   > Upgrading from the pre-2.0 Discord plugin? The redirect URI changed with
   > the plugin id — the old `…/oauth/discord/callback` entry no longer matches.
...
The button face is drawn by PDK: the microphone or headphone glyph on Discord's
blurple, switching to red the moment you are muted or deafened. Deafening also
flips the mute button, because Discord treats a deafened mic as muted.

The plugin polls Discord's voice state every 5 seconds, so the buttons stay in
sync even when you mute or deafen from inside Discord itself.

When something isn't ready, the face turns dark grey with a short hint instead
of a state icon:

| Hint | Meaning |
|---|---|
| `SETUP` | No Client ID / Secret saved yet — see Step 4 |
| `AUTH` | Credentials saved but not authorized — see Step 5 |
| `OFF` | Authorized, but the Discord desktop app isn't reachable |
...
| Error | Cause | Fix |
|---|---|---|
| `redirect_uri: Not matching configuration` | Redirect URI in Discord portal doesn't match | Ensure it is exactly `http://127.0.0.1:8686/oauth/no.pydeck.discord/callback` |
| `Discord IPC socket not found` | Discord is not running, or you are using a browser / modified client | Start the **official** Discord desktop app; browser and modified clients (Vencord, BetterDiscord) do not expose the IPC endpoint |
| `Authentication failed` | Token is invalid or expired | Go to Settings → Credentials → Discord and click **Authorize** again |
| `AUTH` on the button | No token saved yet | Complete Step 5 (Authorize) |
| Button face stuck after mute inside Discord | Poll hasn't fired yet | State syncs within ~5 s |
