...
def _resolve_redirect_uri() -> str:
    ...
    try:
        from lib.plugins import oauth as _oauth_lib  # noqa: PLC0415
...
def _socket_is_live(rpc: DiscordRPC) -> bool:
    """True when the authenticated connection already exists (no I/O performed)."""
...
def load_credentials(ctx: Any = None) -> Dict[str, Any]:
    ...
    The core's store is the source (it merges the pre-RDNN slug and the RDNN
    id itself); ``ctx.credentials`` overlays it when present. On a core that
    still keeps credentials.json the file is read directly instead.
    """
    merged: Dict[str, Any] = {}
    store = _core_store()
    try:
