"""Toggle Deafen — toggle Discord deafen (mic + audio) via the local RPC socket."""
...
_ICON_LIVE = "assets/icons/headphones.png"
_ICON_DEAFENED = "assets/icons/headphones_off.png"
...
def _apply(ctx: Any, voice: dict | None) -> None:
    ...
    if voice is None:
        ...
        ctx.state._template = "toggle_deafen-hint"
        return

    deafened = bool(voice.get("deaf"))
    ctx.state.icon_src = (
        _icon(ctx, "active", _ICON_DEAFENED) if deafened
        else _icon(ctx, "default", _ICON_LIVE)
    )
    ctx.state.face_class = "face-active" if deafened else ""
    ctx.state.hint = ""
    ctx.state._template = "toggle_deafen"
...
def on_press(ctx: Any) -> None:
    try:
        state = get_rpc(ctx).toggle_deafen()
    except Exception:
        evict_rpc()
        _apply(ctx, None)
        return
