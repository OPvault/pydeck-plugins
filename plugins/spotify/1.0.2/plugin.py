...
# Last album-art URL, track label, and time-left string applied per deck slot.
# Stored as (art_url, label, time_left_str) — None means not yet sent.
_last_spotify_face: dict[tuple[str, int], tuple[Optional[str], Optional[str], Optional[str]]] = {}
...
def poll_display(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(bid, int):
        bid = 0
    key = (did, bid)
    prev_art_sent, prev_label_sent, prev_time_left_sent = _last_spotify_face.get(key, (None, None, None))
