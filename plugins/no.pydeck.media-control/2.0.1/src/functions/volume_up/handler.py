"""Volume Up — raises the default sink by a configurable step."""
...
_ICON = "assets/icons/VolumeUp.png"
...
def on_press(ctx: Any) -> None:
    shared = _shared(ctx)
    shared.volume("volume_up", shared.clamp_step(ctx.config.get("step_percent", 5)))
    _paint(ctx)
