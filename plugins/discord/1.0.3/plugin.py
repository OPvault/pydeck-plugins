...
from discord_rpc import DiscordRPC, DiscordRPCError, _DEFAULT_REDIRECT_URI  # noqa: E402


def _resolve_redirect_uri() -> str:
    """Return the redirect URI from lib.oauth so it stays in sync with the
    server's configured port.  Falls back to the constant in discord_rpc if
    lib is not importable (e.g. during isolated testing)."""
    try:
        from lib import oauth as _oauth_lib  # noqa: PLC0415
        return _oauth_lib.get_redirect_uri("discord")
    except Exception:
