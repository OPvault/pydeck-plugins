...
# The verification context that last worked, so the fallback search below is
# paid for once per session rather than on every poll.
...
# Windows is where these hosts routinely fail to verify.
# OpenSSL — which is what Python's ssl module is — can only see the roots
# CryptoAPI happens to have cached; unlike schannel it cannot trigger Windows'
# on-demand root download.  api.met.no chains to "HARICA TLS RSA Root CA 2021",
# an anchor a fresh Windows install has not fetched yet, so every request dies
# with ``certificate verify failed: unable to get local issuer certificate``
# while the very same URL loads fine in a browser.  The mirror-image failure
# (an *expired* anchor in the CA store aborting an otherwise valid chain) is
# the one PyDeck's own marketplace hit, so both are handled here.
...
def _urlopen(req: "urllib.request.Request", timeout: float = 10):
...
def current_conditions(
    ...
    Raises ValueError on an empty response rather than reporting a 0 °C
    reading the API never gave us.
    """
    if not timeseries:
