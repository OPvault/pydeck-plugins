...
- **Steamworks SDK** — the `libsteam_api.so` the Steam client ships is loaded
  for `SteamAPI_IsSteamRunning`. Set `STEAMWORKS_SDK=/path/to/sdk` to point
  the plugin at a downloaded SDK instead. The SDK has no call that lists a
  user's library, so the manifest files above remain the source of truth.

## Steam locations checked

`~/.local/share/Steam`, `~/.steam/steam`, `~/.steam/root`, the Flatpak
`com.valvesoftware.Steam` data dirs, or whatever `STEAM_ROOT` points at.
