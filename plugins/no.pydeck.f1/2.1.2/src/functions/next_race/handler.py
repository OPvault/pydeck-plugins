...
schedule_unavailable = _shared.schedule_unavailable
...
def _poll(ctx: Any) -> None:
    ctx.state._template = "next_race"
    meeting = get_or_refresh_meeting(ctx.storage_path)

    if meeting is None:
        # No meeting can mean two very different things.  Only say "Off
        # Season" when a calendar was actually read and had nothing left
        # in it; a failed fetch is a temporary "No Data".
        ctx.state.countdown = (
            "No Data" if schedule_unavailable() else "Off Season"
        )
        ctx.state.countdown_class = "countdown"
        ctx.state.session_label = ""
        ctx.state.track_label = ""
