...
def icon_src(ctx: Any, entity_id: str, state: str, attrs: dict) -> str:
    """Rasterize the entity's icon and return a storage-relative src."""
    name = resolve_icon(entity_id, state, attrs)
    color = ICON_ON_COLOR if is_on(state) else ICON_OFF_COLOR
...
def format_value(state: str, attrs: dict) -> str:
    """Render a sensor value with its unit, if it has one."""
    unit = str(attrs.get("unit_of_measurement") or "").strip()
    text = str(state or "").strip()
    if not text:
        return ""
    if unit:
        return f"{text}{'' if unit.startswith('°') else ' '}{unit}"
    return text.replace("_", " ").title() if text.isalpha() else text
