...
def load_credentials(ctx: Any = None) -> Dict[str, Any]:
    ...
    The core's store is the source; ``ctx.credentials`` overlays it when
    present. "ha" is an older hand-written key the core's alias map does not
    know, so it is read explicitly, RDNN last so a migrated blob wins. On a
    core that still keeps credentials.json the file is read directly instead.
    """
    merged: Dict[str, Any] = {}
    store = _core_store()
    try:
        if store is not None:
            for key in (*_LEGACY_PLUGIN_IDS, PLUGIN_ID):
                merged.update({k: v for k, v in store.load(key).items() if v})
