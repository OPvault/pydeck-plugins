"""Home Assistant REST API client and MDI icon renderer for PyDeck (PDK).
...
try:
    ...
    # Not just ImportError: on Windows cairosvg imports cleanly and then
    # cairocffi raises OSError when it cannot dlopen libcairo-2.dll, which
    # is not shipped with the platform.  Either way we fall back to the
    # Pillow icons below rather than taking the whole module down.
    _cairosvg = None
    _HAS_CAIROSVG = False
...
class HaClient:
    ...
    def get_state(self, entity_id: str) -> dict:
        try:
            ...
            # /api/states/<id> 404s for an unknown entity too, so the generic
            # "check your URL" message would point at the wrong problem.
            if "404" in str(e):
                raise HaClientError(f"Unknown entity: {entity_id}") from e
            raise
...
def render_entity_icon(
    ...
    """Rasterize an MDI icon to a transparent PNG under *storage_dir*.

    PDK draws the button face itself, so this only produces the icon --
    no bottom margin for a title, and the caller picks the tint.

    Returns a path **relative to storage_dir** for use in ``<img src>``
    (e.g. ``icons/_icon_ab12cd34ef56.png``), or "" if rendering fails.
    """
    try:
        norm_name = _normalize_icon_name(icon_name)
        cache_hash = hashlib.md5(
            f"{norm_name}:{size}:{color}".encode()
        ).hexdigest()[:12]
        rel_path = f"icons/_icon_{cache_hash}.png"
        ...
        icon_img = svg_to_pil(
            svg, size=size, hex_color=color, icon_name=norm_name
        )
        if icon_img is None:
            return ""
        img.paste(
            icon_img,
            ((size - icon_img.width) // 2, (size - icon_img.height) // 2),
            icon_img,
        )
