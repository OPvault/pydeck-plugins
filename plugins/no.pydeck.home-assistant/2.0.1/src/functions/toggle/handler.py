...
def _render(ctx: Any, entity_id: str, state: str, attrs: dict) -> None:
    ...
    show_icon = bool(ctx.config.get("show_icon", True))
    show_state = bool(ctx.config.get("show_state", True))
    highlight = bool(ctx.config.get("highlight_on", True))

    ctx.state._template = "toggle" if show_label else "toggle-nolabel"
    ctx.state.error = ""

    # An empty <text> and a 0x0 <img> both collapse out of the layout, so the
    # icon and state rows are hidden from state -- only <buttonlabel> needs a
    # separate template.
    if show_icon:
        icon_px = shared.icon_size_for(int(show_label) + int(show_state))
        ctx.state.icon_src = shared.icon_src(
            ctx, entity_id, state, attrs, highlight=highlight
        )
    ...
    if not show_state:
        ...
        ctx.state.state_class = (
            "state-on" if (on and highlight) else "state-off"
        )
